...
#else
      Mascot->mail.file=g_strconcat(get_home_dir(),
#endif
				    G_DIR_SEPARATOR_S, 
				    "Maildir", 
				    G_DIR_SEPARATOR_S, 
...
#else
	tmp_file2=g_strconcat(get_home_dir(), G_DIR_SEPARATOR_S,USER_DIR,
#endif
...
#else
  userdir = g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,USER_DIR, NULL);
#endif
...
#else
  userdir = g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,USER_DIR,PIXDIR, NULL);
#endif
...
#else
  userdir = g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,USER_DIR,SOUNDDIR, NULL);
#endif
...
#else
    dir = g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,
		      USER_DIR,PIXDIR,NULL);
#endif
    break;
  case FOLDER_SOUND:
...
#else
      tmp_file1 = g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,USER_DIR, NULL);
#endif
#ifdef USE_COMMON
...
#else
    tmp_file=g_strconcat(get_home_dir(), G_DIR_SEPARATOR_S,USER_DIR,
#endif
			 PIXDIR, NULL);
...
#else
      filepath=g_strconcat(get_home_dir(), G_DIR_SEPARATOR_S,USER_DIR, NULL);
#endif
...
void create_config_dialog(void)
{
  ...
  // ノートブックの生成
  {
	...
	adj_pop_port = (GtkAdjustment *)gtk_adjustment_new 
	  ((gfloat)Mascot->mail.pop_port, 1, 65535, 1.0, 100.0, 100.0);
	...
	adj = (GtkAdjustment *)gtk_adjustment_new 
	  ((gfloat)Mascot->mail.pop_max_fs, 10, 999, 10.0, 10.0, 10.0);
	my_signal_connect (adj, "value_changed",cc_get_adj,
	...
	gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(combo)->entry),
      ...
      adj = (GtkAdjustment *)gtk_adjustment_new 
	((gfloat)Mascot->sockmsg_step, 0,10, 1.0, 1.0, 1.0);
      my_signal_connect (adj, "value_changed",cc_get_adj,
...
void make_pixmap_list(GtkWidget *scrwin)
{
  ...
  for(i_pix=0;i_pix<tmp_mascot.nPixmap;i_pix++){
    ...
    my_signal_connect(button,"clicked",TestPut, (gpointer)(i_pix));
...
#endif
  gtk_box_pack_start(GTK_BOX(hbox), button,FALSE, FALSE, 0);
  my_signal_connect(button,"clicked",create_add_frame_dialog,(gpointer)i_ptn);
...
#endif
  gtk_box_pack_start(GTK_BOX(hbox), button,FALSE, FALSE, 0);
  my_signal_connect(button,"clicked",create_del_frame_dialog,(gpointer)i_ptn);
...
#endif
  gtk_box_pack_start(GTK_BOX(hbox), button,FALSE, FALSE, 0);
  my_signal_connect(button,"clicked",TestAnime,(gpointer)i_ptn);
...
void make_cat_list(GtkWidget *cat_note, int i_cat)
{
  ...
  // カテゴリー名
  {
	...
	gtk_table_attach(GTK_TABLE(cat_table[i_cat]), button,
			 0, 1, 0, 1,
			 GTK_SHRINK,GTK_SHRINK,0,0);
	my_signal_connect (button,"clicked",mc_up_cat,(gpointer)i_cat);
      }
	...
	gtk_table_attach(GTK_TABLE(cat_table[i_cat]), button,
			 3, 4, 0, 1,
			 GTK_SHRINK,GTK_SHRINK,0,0);
	my_signal_connect (button,"clicked",mc_down_cat,(gpointer)i_cat);
      }
...
#endif
  gtk_box_pack_start(GTK_BOX(hbox), button,FALSE, FALSE, 0);
  my_signal_connect(button,"clicked",create_add_tgt_dialog,(gpointer)i_cat);
...
#endif
  gtk_box_pack_start(GTK_BOX(hbox), button,FALSE, FALSE, 0);
  my_signal_connect(button,"clicked",create_del_tgt_dialog,(gpointer)i_cat);
...
#else
	mascot->menu_file=g_strconcat(get_home_dir(),G_DIR_SEPARATOR_S,USER_DIR,
#endif
...
void do_install_user_menu(GtkWidget *w, gpointer gdata){
  ...
  create_menu_selection_dialog(NULL,(gpointer)START_MENU_INSTALL_USER);
...
void do_install_common_menu(GtkWidget *w, gpointer gdata){
  ...
  create_menu_selection_dialog(NULL,(gpointer)START_MENU_INSTALL_COMMON);
...
void do_install_user_mascot(GtkWidget *w, gpointer gdata){
  ...
  create_file_selection_dialog(NULL,(gpointer)START_MENU_INSTALL_USER);
...
void do_install_common_mascot(GtkWidget *w, gpointer gdata){
  ...
  create_file_selection_dialog(NULL,(gpointer)START_MENU_INSTALL_COMMON);
...
void do_select_mascot(GtkWidget *w, gpointer gdata){
  ...
  create_file_selection_dialog(NULL,(gpointer)START_MENU_SELECT);
...
void popup_message(gint delay, ...){
  ...
  my_signal_connect(dialog,"destroy",destroy_popup, (gpointer *)timer);
